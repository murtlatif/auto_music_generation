...
def spectrogram(waveform, sr, chunk_size_s=None, overlap=0, *, n_mels=80, n_fft=270):
    ...
    for idx, chunk in enumerate(chunks):
        print(f'Processing chunk {idx+1}/{len(chunks)}. Shape: {chunk.shape}')
        if idx == 0 or idx >= len(chunks):
            ...
            print('C', missing_len)
            print(chunk.shape, chunk[-missing_len:].shape, len(chunk))
    ...
    specgram = torchaudio.transforms.MelSpectrogram(
        n_mels=n_mels, n_fft=n_fft)(waveforms)
    return specgram
...
def get_dataset(file_path, net_state_path, chunk_size_s, overlap, n_mels=80, n_fft=270):
    ...
    specs = spectrogram(waveform, sr, chunk_size_s=chunk_size_s,
                        overlap=overlap, n_mels=n_mels, n_fft=n_fft)
    ...
    for spec_data in spec_datas:
        label = get_label(
            spec_data, net_state_path=net_state_path, n_mels=n_mels, n_fft=n_fft)
